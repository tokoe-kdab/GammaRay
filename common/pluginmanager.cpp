...
void PluginManagerBase::scan(const QString &serviceType)
{
  ...
  foreach (const auto &staticPlugin, QPluginLoader::staticPlugins()) {
    ...
    if (!pluginInfo.isValid() || loadedPluginNames.contains(pluginInfo.id()) || pluginInfo.interface() != serviceType) {
      qDebug() << "skipping static plugin " << pluginInfo.id() << pluginInfo.interface();
      continue;
    }
